...
#   frontend/sender-app/src/contracts.ts          (generated, sender app)
#   frontend/agent-dashboard/src/contracts.ts     (generated, agent dashboard)
#
# Usage:
...
# Regenerate the sender-app's and agent-dashboard's contract wiring so both
# frontends talk to these exact testnet instances (this is what keeps the
# sender → agent loop on the same contract set).
for FRONTEND in sender-app agent-dashboard; do
  FRONTEND_SRC="${REPO_ROOT}/frontend/${FRONTEND}/src/contracts.ts"
  [[ -d "${REPO_ROOT}/frontend/${FRONTEND}/src" ]] || continue
  cat > "${FRONTEND_SRC}" <<EOF
// DO NOT EDIT — generated by scripts/deploy.sh. Re-run to point the ${FRONTEND}
